...
class SearchForm(forms.Form):
    q = forms.CharField(required=False, label=False,
                        widget=forms.TextInput(attrs={'type': 'search', 'class': 'form-control', 'id': 'searchbar'}))
...
class ModelSearchForm(SearchForm):
    def __init__(self, *args, **kwargs):
        ...
        # self.fields['models'] = forms.MultipleChoiceField(choices=model_choices(), required=False, label=None,
    ...
    def search(self):
        ...
        # sqs = self.searchqueryset.filter(SQ(content=AutoQuery(self.cleaned_data['q'])) | SQ(title=AutoQuery(self.cleaned_data['q'])))
